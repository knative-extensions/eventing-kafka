...
func TestGetConfigFromSecret_Valid(t *testing.T) {
	...
	kafkaAuth := GetConfigFromSecret(secret)
...
func TestGetConfigFromSecret_Invalid(t *testing.T) {
	assert.Nil(t, GetConfigFromSecret(nil))
}
...
func TestInitializeSecretWatcher(t *testing.T) {
	...
	// Create A Test Auth Secret For The InitializeSecretWatcher() Call To Watch
	secret := commontesting.GetTestSaramaSecret(
		commontesting.OldAuthBrokers,
		commontesting.OldAuthUsername,
		commontesting.OldAuthPassword,
	...
	// Create The Fake K8S Client And Add It To The Secret
	fakeK8sClient := fake.NewSimpleClientset(secret)

	// Add The Fake K8S Client To The Context (Required By InitializeSecretWatcher)
