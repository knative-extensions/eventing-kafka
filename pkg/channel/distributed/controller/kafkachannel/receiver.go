...
func (r *Reconciler) newReceiverDeployment(secret *corev1.Secret) *appsv1.Deployment {
	...
	replicas := int32(r.config.Channel.Distributed.Receiver.Replicas)
	...
	if !r.config.Channel.Distributed.Receiver.MemoryLimit.IsZero() {
		resourceLimits[corev1.ResourceMemory] = r.config.Channel.Distributed.Receiver.MemoryLimit
	}
	if !r.config.Channel.Distributed.Receiver.CpuLimit.IsZero() {
		resourceLimits[corev1.ResourceCPU] = r.config.Channel.Distributed.Receiver.CpuLimit
	}
	resourceRequests := make(map[corev1.ResourceName]resource.Quantity)
	if !r.config.Channel.Distributed.Receiver.MemoryRequest.IsZero() {
		resourceRequests[corev1.ResourceMemory] = r.config.Channel.Distributed.Receiver.MemoryRequest
	}
	if !r.config.Channel.Distributed.Receiver.CpuRequest.IsZero() {
		resourceRequests[corev1.ResourceCPU] = r.config.Channel.Distributed.Receiver.CpuRequest
	}
