...
readonly STRIMZI_INSTALLATION_CONFIG_TEMPLATE="test/config/100-strimzi-cluster-operator-0.25.0.yaml"
...
function test_mt_source() {
  ...
  echo "Run rekt tests"
  go_test_e2e -tags=e2e -timeout=80m -test.parallel=${TEST_PARALLEL} -run "^TestKafkaSource*" ./test/rekt/... || fail_test
