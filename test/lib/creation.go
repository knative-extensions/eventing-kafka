...
func CreateKafkaChannelV1Alpha1OrFail(c *testlib.Client, kafkaChannel *channelsv1alpha1.KafkaChannel) {
	kafkaChannelClientSet, err := kafkaclientset.NewForConfig(c.Config)
	if err != nil {
		c.T.Fatalf("Failed to create v1alpha1 KafkaChannel client: %v", err)
	}

	kChannels := kafkaChannelClientSet.MessagingV1alpha1().KafkaChannels(c.Namespace)
	if createdKafkaChannel, err := kChannels.Create(context.Background(), kafkaChannel, metav1.CreateOptions{}); err != nil {
		c.T.Fatalf("Failed to create v1alpha1 KafkaChannel %q: %v", kafkaChannel.Name, err)
	} else {
		c.Tracker.AddObj(createdKafkaChannel)
	}
}

func GetKafkaChannelV1Alpha1OrFail(c *testlib.Client, kafkaChannel string) *channelsv1alpha1.KafkaChannel {
	kafkaChannelClientSet, err := kafkaclientset.NewForConfig(c.Config)
	if err != nil {
		c.T.Fatalf("Failed to create v1alpha1 KafkaChannel client: %v", err)
	}
	...
	if kcObj, err := kChannels.Get(context.Background(), kafkaChannel, metav1.GetOptions{}); err != nil {
		c.T.Fatalf("Failed to get v1alpha1 KafkaChannel %q: %v", kafkaChannel, err)
	} else {
		return kcObj
	}
	return nil
